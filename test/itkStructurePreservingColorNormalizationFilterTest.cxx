...
int
itkStructurePreservingColorNormalizationFilterTest(int argc, char * argv[])
{
  {
  ...
  // { std::ostringstream mesg; mesg << "logHematoxylin = " << logHematoxylin << ", logEosin = " << logEosin <<
  // std::endl; std::cout << mesg.str(); }
  ...
  for (ImageType::Pointer image : { input, refer })
  {
    ...
    for (iter.GoToBegin(); !iter.IsAtEnd(); ++iter)
    {
      ...
      const CalcRowVectorType randomPixelValue{ (logWhite - (hematoxylinContribution * logHematoxylin) -
                                                 (eosinContribution * logEosin))
                                                  .unaryExpr(CalcUnaryFunctionPointer(std::exp))
                                                  .array() +
                                                noise };
      // std::cout << "hematoxylinContribution = " << hematoxylinContribution << ", eosinContribution = " <<
      // eosinContribution << ", randomPixelValue = " << randomPixelValue << std::endl;
      for (int color{ 0 }; color < NumberOfColors; ++color)
      {
